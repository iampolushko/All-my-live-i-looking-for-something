...
Оболочка Bash - это командный интерпретатор, содержащий в себе текстовый интерфейс для работы с ОС и язык программирования для автоматизации администрирования.

```<tab>``` нажми, когда вводишь команду, чтобы посмотреть все похожие.

```history``` - чтобы посмотреть полную историю команд.

При выполнении команд можно использовать **переменные среды**, например ```cd $HOME```.



### Ошибка файловой системы
Её основные симптомы - это если все файлы в сестеме становятся только для чтения, а при запуске открывается gnu grub.

Для её решения нужно в продвинутый настройках открыть `initramfs` терминал(выбрать "Ubuntu recovery mod" в настройках запуска) и ввести следующие комманды:

1)Узнать имя корневого раздела
```bash
blkid
```
нужно выбрать раздел с `TYPE="ext4"` или с `TYPE="ext3"`. На NVMe дисках он как правило называется `/dev/sda1` или `/dev/sda2`

2)Запустить проверку и починку
```bash
e2fsck -f -y /dev/sda2
```
`-f` — принудительная проверка, даже если раздел кажется чистым.

`-y` — автоматический ответ "да" на все вопросы об исправлении ошибок.

После выполнения нужно увидеть сообщение `FILE SYSTEM WAS MODIFIED`

3)Выходим из initramfs
Вводим команду:
```bash
exit
```
Если сразу после выхода у нас открывается `Recovery Menu` с графическим интерфейсом(обычно розовым фоном), то мы можно просто выйти из него. Всё должно заработать
