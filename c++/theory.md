...
### Общие типы данных(TCHAR, WCHAR, LPSTR, LPWSTR,LPCTSTR)
#### THAR 
Чтобы код был независимым от разных типов кодировок, его можно писать в общей манере кодировок.

В настройках проекта как правило можно указать, в какой кодировке будет компилироваться программа. Например в visual studio на вкладке **General** можно поменять параметр **Character Set** на ```Use Unicode Character set``` или ```Use Multi-Byte Character Set```. 

Чтобы THAR работал нужно подрубить либо ```#include <TCHAR.H>.``` либо ```#include <windows.h>``` 

Если указан параметр «Use Unicode Character set», тип TCHAR будет транслироваться в тип wchar_t(2 байта). Если указан параметр «Use Multi-byte character set» то тогда TCHAR будет транслироваться в тип char(1 байта).

Чтобы в самом коде сменить тип данных у строки или символа в самок коде, то:
```c++
...
_T("Cтрока, зависящая от компиляции"); // THAR ANSI или Unicode
```
TCHAR это макрос, для декларирования одного символа. Вы также можете декларировать массив TCHAR.

#### LPCSTR

LPCSTR понимается как: 
...
### В с++ можно создавать функции или методы с параметрами по умолчанию
```c++
#include <iostream>

// Объявляем функцию с параметрами по умолчанию
...
int main()
{
    std::cout << add(5) << std::endl;        // 5 + 10 + 20 = 35
    std::cout << add(5, 2) << std::endl;     // 5 + 2 + 20 = 27
    std::cout << add(5, 2, 3) << std::endl;  // 5 + 2 + 3  = 10

    return 0;
}
```

const после названия метода, значит, что метод не может менять свойства(поля) класса.

конструктор по умолчанию - это конуструктор, не принимающий никаких аргументов. Он создаётся автоматически

const после функции значит, что она не может менять данные. Для константных объектов нужно вызывать константные методы


Кроме конструктора есть и диструктор