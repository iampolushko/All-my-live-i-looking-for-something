...
Данный способ не подходит для потоковой передачи, так как readAll будет пытаться прочитать все данные, которые приходят бесконечно.
У QSerialPort есть buffer. Это штука позволяет читать строго определённое количество бит. Если мы не укажем его вручную, то это произойдёт автоматически. 
Так же, при попытке прочитать данные заново, нам нужно очистить buffer. 
```c++
qint64 numReadTotal = 0; //Этот подсчёт здесь просто для примера
char buffer[50]; // размер buffer'а можно менять.
...
for (;;) {
    ...
    if (numRead == 0 && !serial.waitForReadyRead()) //Или любое другое условие остановки
        break;
